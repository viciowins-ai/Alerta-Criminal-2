    ...
    const renderContactItem = ({ item }) => (
        <View className="flex-row items-center bg-slate-800/60 p-4 mb-3 rounded-2xl border border-slate-700/50 shadow-sm">
            ...
            <View className="flex-1">
                ...
                {item.relationship && (
                    <Text className="text-blue-400 text-xs mt-1 font-medium bg-blue-500/10 self-start px-2 py-0.5 rounded-full">{item.relationship}</Text>
                )}
            </View>
            <TouchableOpacity
            ...
            <View className="flex-1 w-full h-full">
                ...
                <View className="flex-1 px-6 pt-6">
                    <View className="bg-red-900/20 p-4 rounded-2xl border border-red-900/30 mb-6 flex-row gap-3">
                        <MaterialIcons name="info-outline" size={24} color="#f87171" className="mt-0.5" />
                        <Text className="text-red-200 text-sm flex-1 leading-5">
                            Esses contatos serão notificados <Text className="font-bold text-red-100">automaticamente e instantaneamente</Text> quando você acionar o botão SOS (Pânico). Mantenha esta lista atualizada.
                        </Text>
                    </View>
                        ...
                        ListEmptyComponent={() => (
                            <View className="items-center justify-center py-20 opacity-50">
                                <View className="w-20 h-20 bg-slate-800/50 rounded-full items-center justify-center mb-4">
                                    <MaterialIcons name="contact-phone" size={40} color="#64748b" />
                                </View>
                                <Text className="text-slate-500 mt-2 text-center text-base">Nenhum contato adicionado.</Text>
                            </View>
                        )}
                    />
                </View>
                    ...
                    <View className="flex-1 justify-end bg-black/80 backdrop-blur-sm">
                        <View className="bg-slate-900 rounded-t-[2.5rem] p-8 border-t border-slate-700 shadow-2xl">
                            <View className="flex-row justify-between items-center mb-6">
                                <Text className="text-2xl font-bold text-white">Novo Contato</Text>
                                <TouchableOpacity onPress={() => setModalVisible(false)} className="bg-slate-800 p-2 rounded-full">
                            ...
                            <View className="space-y-5">
                                <View>
                                    <Text className="text-slate-400 text-xs font-bold mb-2.5 ml-1 uppercase tracking-wider">Nome</Text>
                                    <TextInput
                                ...
                                <View className="mt-4">
                                    <Text className="text-slate-400 text-xs font-bold mb-2.5 ml-1 uppercase tracking-wider">Telefone</Text>
                                    <TextInput
                                        className="bg-slate-800/80 text-white p-4 rounded-2xl border border-slate-700 focus:border-blue-500 transition-colors"
                                ...
                                <View className="mt-4">
                                    <Text className="text-slate-400 text-xs font-bold mb-2.5 ml-1 uppercase tracking-wider">Parentesco (Opcional)</Text>
                                    <TextInput
                                        className="bg-slate-800/80 text-white p-4 rounded-2xl border border-slate-700 focus:border-blue-500 transition-colors"
                                ...
                                >
                                    <Text className="text-white font-bold text-lg tracking-wide">Adicionar Contato</Text>
                                </TouchableOpacity>
                                <View className="h-4" />
                            </View>
                        </View>
