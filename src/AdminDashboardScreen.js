...
const AdminDashboardScreen = ({ navigation }) => {
  ...
  const dashboardData = {
    "7d": {
      ...
      trend: "+12%",
      chartPath:
        "M0 100 C 50 100 50 50 100 50 C 150 50 150 80 200 80 C 250 80 250 20 300 20 C 350 20 350 90 400 90 L 400 150 L 0 150 Z",
    },
    "30d": {
      total: "1,240",
      verified: "980",
      trend: "+15%",
      chartPath:
        "M0 109C18 109 18 21 36 21C54 21 54 41 72 41C90 41 90 93 108 93C127 93 127 33 145 33C163 33 163 101 181 101C199 101 199 61 217 61C236 61 236 45 254 45C272 45 272 121 290 121C308 121 308 149 326 149C344 149 344 1 363 1C381 1 381 81 399 81C417 81 417 129 435 129C453 129 453 25 472 25V149H0V109Z",
    },
    "90d": {
      total: "3,540",
      verified: "3,100",
      trend: "+22%",
      chartPath:
        "M0 120 C 60 120 60 60 120 60 C 180 60 180 100 240 100 C 300 100 300 10 360 10 C 420 10 420 80 480 80 L 480 150 L 0 150 Z",
    },
  };
  ...
  const KPICard = ({ title, value, icon, iconColor, trend, isNegative }) => (
    <View className="flex-1 bg-slate-800/80 border border-slate-700 p-4 rounded-2xl min-w-[45%] mb-3">
      <View className="flex-row justify-between items-start mb-2">
        <View
          className={`w-10 h-10 rounded-xl items-center justify-center bg-slate-700/50`}
        >
          <MaterialIcons name={icon} size={20} color={iconColor} />
        </View>
        {trend && (
          <View
            className={`px-2 py-1 rounded-full ${isNegative ? "bg-red-500/20" : "bg-green-500/20"}`}
          >
            <Text
              className={`text-xs font-bold ${isNegative ? "text-red-400" : "text-green-400"}`}
            >
              {trend}
            </Text>
          </View>
  ...
  const renderHeader = (title) => (
    <View className="flex-row items-center justify-between px-6 py-4 bg-background-dark border-b border-slate-800 sticky top-0 z-50">
      ...
      <Text className="text-lg font-bold text-white tracking-wide uppercase">
        {title}
      </Text>
        ...
        <View className="flex-row justify-between items-center mb-6">
          ...
          <View className="flex-row bg-slate-700/50 rounded-lg p-1">
            {["7d", "30d", "90d"].map((t) => (
              ...
              >
                <Text
                  className={`text-xs font-bold ${selectedTimeframe === t ? "text-white" : "text-slate-400"}`}
                >
                  {t}
                </Text>
              </TouchableOpacity>
              ...
              <View className="flex-1 p-4">
                ...
                <View className="flex-row justify-between items-start">
                  <View className="flex-1">
                    ...
                    <Text className="text-slate-400 text-sm mb-0.5">
                      {item.time} • {item.loc}
                    </Text>
    ...
    >
      <View className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden mb-6">
        {[
          ...
          <View
            key={idx}
            className={`flex-row items-center justify-between p-4 ${idx < 2 ? "border-b border-slate-700" : ""}`}
          >
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-dark">
      ...
      <View className="flex-1">
        ...
        {(activeTab === "alertas" || activeTab === "usuarios") &&
        ...
        <View className="absolute bottom-6 left-6 right-6 bg-slate-900/95 border border-slate-700/50 rounded-2xl flex-row items-center justify-between p-2 shadow-2xl backdrop-blur-xl">
          {[
            ...
            >
              ...
              />
              {activeTab === tab.id && (
                <View className="w-1 h-1 rounded-full bg-blue-500 results mt-1" />
              )}
            </TouchableOpacity>
          ))}
        </View>
