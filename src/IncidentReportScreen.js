...
const IncidentReportScreen = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (!user && !isAnonymous) {
            Alert.alert('Atenção', 'Você está navegando como visitante. O relato será anônimo automaticamente.');
        }
        ...
        <SafeAreaView className="flex-1 bg-[#0a0f1c]">
            ...
            <View className="flex-row items-center justify-between px-6 py-4 z-20">
                ...
                <View className="items-center">
                    <Text className="text-white font-black text-[16px] tracking-widest uppercase">Reportar Alerta</Text>
                    <Text className="text-red-400 text-[10px] font-bold tracking-widest uppercase mt-1">Sua voz protege todos</Text>
                </View>
                <View className="w-10" /> {/* Balanço flex */}
            </View>

            <ScrollView className="flex-1 px-6 pt-2" contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
                ...
                <View className="flex-row items-center bg-blue-600/10 border border-blue-500/20 rounded-2xl p-4 mb-6">
                    ...
                    <View className="flex-1">
                        <Text className="text-blue-400 font-bold text-xs uppercase tracking-wider mb-1">Nível 2: Observador</Text>
                        <Text className="text-slate-300 text-[11px] leading-4">Reporte com precisão para ganhar +50 XP e evoluir para o próximo nível.</Text>
                    </View>
                </View>

                {/* Location Map Preview */}
                <View className="flex-row items-centerjustify-between mb-3 mt-2">
                    <Text className="text-white font-extrabold text-[15px] tracking-wide">Ponto de Risco</Text>
                    <Text className="text-slate-500 text-xs ml-auto">Localização Automática</Text>
                </View>
                ...
                <Text className="text-white font-extrabold text-[15px] tracking-wide mb-4">Classificação do Perigo</Text>
                <View className="flex-row flex-wrap justify-between gap-y-4 mb-8">
                    ...
                    >
                        <MaterialIcons name="directions-run" size={28} color={selectedType === 'roubo' ? "white" : "#94a3b8"} />
                        <Text className={`font-bold text-xs uppercase tracking-widest ${selectedType === 'roubo' ? 'text-white' : 'text-slate-400'}`}>Assalto</Text>
                    </TouchableOpacity>
                    ...
                    >
                        <Ionicons name="eye-outline" size={28} color={selectedType === 'suspect' ? "white" : "#94a3b8"} />
                        <Text className={`font-bold text-xs uppercase tracking-widest text-center ${selectedType === 'suspect' ? 'text-white' : 'text-slate-400'}`}>Suspeitos</Text>
                    </TouchableOpacity>
                    ...
                    >
                        <FontAwesome5 name="car-crash" size={24} color={selectedType === 'vandalism' ? "white" : "#94a3b8"} />
                        <Text className={`font-bold text-xs uppercase tracking-widest text-center ${selectedType === 'vandalism' ? 'text-white' : 'text-slate-400'}`}>Acidente</Text>
                    </TouchableOpacity>
                    ...
                    >
                        <MaterialIcons name="add-circle-outline" size={28} color={selectedType === 'other' ? "white" : "#94a3b8"} />
                        <Text className={`font-bold text-xs uppercase tracking-widest ${selectedType === 'other' ? 'text-white' : 'text-slate-400'}`}>Outros</Text>
                    </TouchableOpacity>
                </View>

                {/* Details */}
                <Text className="text-white font-extrabold text-[15px] tracking-wide mb-4">Informações Adicionais</Text>
                ...
                <View className="flex-row items-center justify-between bg-slate-900/50 p-5 rounded-3xl border border-white/5 mb-8">
                    <View className="flex-row items-center flex-1 pr-4">
                        <View className="w-10 h-10 rounded-full bg-slate-800 items-center justify-center mr-4 border border-slate-700">
                            <Ionicons name="incognito" size={20} color={isAnonymous ? "#60a5fa" : "#64748b"} />
                        </View>
                        <View>
                            <Text className="text-white font-bold text-sm">Denúncia Anônima</Text>
                            <Text className="text-slate-500 text-[10px] uppercase mt-1">Seu nome e foto serão ocultados no mapa</Text>
                        </View>
                    </View>
                    <Switch
                ...
                >
                    {loading ? (
                        ...
                        <View className="flex-row items-center gap-3">
                            ...
                            <Text className="text-white font-black text-sm uppercase tracking-[0.2em]">Transmitir Alerta</Text>
                        </View>
                    )}
                </TouchableOpacity>
