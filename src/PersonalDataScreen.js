...
const PersonalDataScreen = ({ navigation }) => {
  ...
  const InputField = ({
    ...
    <View className="mb-5">
      <Text className="text-slate-400 text-xs uppercase font-bold mb-2 ml-1 tracking-wider">
        {label}
      </Text>
      <View
        className={`flex-row items-center px-4 h-14 rounded-2xl border transition-all ${editable ? "bg-slate-800/60 border-slate-700/50 focus:border-blue-500/50 focus:bg-slate-800" : "bg-slate-900/40 border-slate-800/50"}`}
      >
        {icon && (
          <MaterialIcons
            name={icon}
            size={20}
            color={editable ? "#94a3b8" : "#475569"}
      ...
      >
        ...
        >
          ...
          <Text className="text-slate-500 text-xs text-center mt-6 px-4 leading-5">
            Para alterar seu CPF ou outras informações sensíveis, entre em
            contato com o suporte através da Central de Ajuda.
          </Text>
        </ScrollView>
