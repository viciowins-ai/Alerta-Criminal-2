...
import { View, Text, ScrollView, TouchableOpacity, TextInput, ActivityIndicator, FlatList, KeyboardAvoidingView, Platform, Alert } from 'react-native';
    ...
    return (
        <SafeAreaView className="flex-1 bg-background-dark">
            ...
            <View className="flex-row items-center px-4 py-3 bg-slate-900 border-b border-slate-800">
                ...
                <Text className="text-white font-bold text-lg ml-2">Detalhes do Alerta</Text>
            </View>
            ...
            >
                <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
                    ...
                    {post && (
                        <View className="bg-slate-900 mb-4 border-b border-slate-800 pb-4">
                            <View className="p-4 flex-row justify-between items-start mb-2">
                                <View className="flex-row gap-3">
                                    <View className="w-12 h-12 rounded-full bg-slate-700 items-center justify-center border border-slate-600">
                                        <FontAwesome5 name={getIconForType(post.type)} size={20} color="#94a3b8" />
                                    </View>
                                    <View>
                                        ...
                                        <Text className="text-slate-500 text-xs">
                                            {post.profiles?.guardian_level || "Novo Guardião"} • {formatTime(post.created_at)}
                                        </Text>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View className="px-4 mb-4 flex-row items-center gap-1">
                                <MaterialIcons name="location-pinned" size={16} color="#64748b" />
                                <Text className="text-slate-500 text-sm">Lat: {post.latitude.toFixed(4)}, Long: {post.longitude.toFixed(4)}</Text>
                            </View>

                            <View className="flex-row mx-4 py-2 border-t border-slate-800 gap-6">
                                <View className="flex-row items-center gap-2">
                                    <Feather name="thumbs-up" size={18} color="#94a3b8" />
                                    <Text className="text-slate-400 font-bold">{post.likes?.[0]?.count || 0}</Text>
                                </View>
                                <View className="flex-row items-center gap-2">
                                    <Feather name="message-square" size={18} color="#94a3b8" />
                                    <Text className="text-slate-400 font-bold">{comments.length}</Text>
                                </View>
                            </View>
                        </View>
                    )}
                    ...
                    {/* Comments Section */}
                    <Text className="text-slate-400 font-bold px-4 mb-2 uppercase text-xs tracking-wider">Comentários</Text>

                    {comments.length === 0 ? (
                        <View className="py-8 items-center">
                            <Text className="text-slate-600">Seja o primeiro a comentar.</Text>
                        </View>
                    ) : (
                        comments.map((comment) => (
                            <View key={comment.id} className="px-4 py-3 border-b border-slate-800/50 flex-row gap-3">
                                <View className="w-8 h-8 rounded-full bg-slate-700 items-center justify-center">
                                    <Text className="text-white font-bold">{comment.profiles?.full_name?.charAt(0) || '?'}</Text>
                                </View>
                                <View className="flex-1 bg-slate-800/50 p-3 rounded-2xl rounded-tl-none">
                                    <View className="flex-row justify-between items-baseline mb-1">
                                        <Text className="text-white font-bold text-sm">{comment.profiles?.full_name || 'Anônimo'}</Text>
                                        <Text className="text-slate-600 text-[10px]">{formatTime(comment.created_at)}</Text>
                                    </View>
                                    <Text className="text-slate-300 leading-5">{comment.content}</Text>
                                </View>
                            </View>
                        ))
                    )}
                ...
                <View className="p-4 bg-slate-900 border-t border-slate-800 flex-row gap-3 items-center">
                    ...
                    >
                        {submitting ? (
                            ...
                            <Ionicons name="send" size={18} color={newComment.trim() ? "white" : "#475569"} />
                        )}
                    </TouchableOpacity>
                </View>
            </KeyboardAvoidingView>
